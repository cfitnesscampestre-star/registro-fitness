...
function calcTendenciaAforo() {
  ...
  const semActStr=fechaLocalStr(semAct);
  const finActStr=fechaLocalStr(finAct);
  const semAntStr=fechaLocalStr(semAnt);
  const finAntStr=fechaLocalStr(finAnt);
...
function calcularAlertas() {
  ...
  Object.values(gruposClase).forEach(g => {
    const recientes = g.recs.filter(r=>(r.fecha||'')>=hoy30str).sort((a,b)=>b.fecha.localeCompare(a.fecha));
    if(recientes.length>=3) {
      ...
      if(bajos.length>=3) {
        ...
        // Encontrar el día de esta clase para el drilldown
        const diaClase = bajos[0].dia || '';
        alertas.push({
          tipo:'red',
          icon:'<svg class="ico" viewBox="0 0 20 20"><circle cx="10" cy="10" r="5" fill="var(--red2)"/></svg>',
          texto:`<strong>${g.clase}</strong> con ${inst?.nombre||'?'} a las ${g.hora} lleva 3+ sesiones consecutivas con aforo menor al 20%.`,
          accion:'Considera revisar o reasignar este horario.',
          drillDown: diaClase ? `verDiagHorario('${diaClase}','${g.hora}')` : `navegarA('heatmap')`
        });
      }
    }
  });
  ...
  instructores.forEach(inst => {
    const faltas = registros.filter(r=>r.inst_id===inst.id&&r.estado==='falta'&&(r.fecha||'')>=hoy30str);
    if(faltas.length>=2) {
      alertas.push({
        tipo:'gold',
        icon:'<svg class="ico ico-warn" viewBox="0 0 20 20"><path d="M10 3 L18 17 H2 Z" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linejoin="round"/><line x1="10" y1="9" x2="10" y2="13" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><circle cx="10" cy="15.5" r="0.8" fill="currentColor"/></svg>⚠',
        texto:`<strong>${inst.nombre}</strong> tiene <strong>${faltas.length} faltas</strong> en los últimos 30 días.`,
        accion:'Requiere seguimiento de RRHH o coordinación.',
        drillDown: `_abrirDetalleInstructor(${inst.id})`
      });
    }
  });
  ...
  instructores.forEach(inst => {
    (inst.horario||[]).forEach(h => {
      ...
      if(!tieneReg) sinRegistro.push({label:`${h.clase} (${inst.nombre.split(' ')[0]}) ${h.dia} ${h.hora}`, inst_id:inst.id, dia:h.dia, hora:h.hora});
    });
  });
  if(sinRegistro.length>0){
    const ejemplos = sinRegistro.slice(0,3).map(x=>x.label).join(', ');
    alertas.push({
      tipo:'blue',
      icon:'<svg class="ico" viewBox="0 0 20 20"><rect x="5" y="3" width="10" height="14" rx="2" stroke="currentColor" stroke-width="1.5" fill="none"/><rect x="8" y="2" width="4" height="2.5" rx="1" stroke="currentColor" stroke-width="1.2" fill="none"/><line x1="8" y1="9" x2="12" y2="9" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><line x1="8" y1="12" x2="13" y2="12" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>',
      texto:`<strong>${sinRegistro.length} clase(s)</strong> de esta semana sin registro aún: ${ejemplos}${sinRegistro.length>3?` y ${sinRegistro.length-3} más`:''}`,
      accion:'Verifica si se impartieron y regístralas.',
      drillDown: `navegarA('clase')`
    });
  }
  ...
  for(let i=3;i>=0;i--){
    ...
    const iniStr = fechaLocalStr(ini);
    const finStr = fechaLocalStr(fin);
    const regs = registros.filter(r=>(r.estado==='ok'||r.estado==='sub')&&r.cap>0&&(r.fecha||'')>=iniStr&&(r.fecha||'')<finStr);
    if(regs.length>0){
  ...
  if(aforoSemanas.length>=3){
    ...
    if(tendencia < -10){
      alertas.push({
        tipo:'red',
        icon:'<svg class="ico" viewBox="0 0 20 20"><polyline points="3,5 7,12 11,9 17,15" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/><polyline points="13,15 17,15 17,11" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        texto:`El aforo promedio ha bajado <strong>${Math.abs(Math.round(tendencia))}%</strong> en las últimas semanas (${aforoSemanas[0]}% → ${aforoSemanas[aforoSemanas.length-1]}%).`,
        accion:'Analiza el mapa de calor para identificar horarios problemáticos.',
        drillDown: `navegarA('heatmap')`
      });
    } else if(tendencia > 5){
      alertas.push({
        tipo:'green',
        icon:'<svg class="ico" viewBox="0 0 20 20"><polyline points="3,15 7,8 11,11 17,5" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/><polyline points="13,5 17,5 17,9" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        texto:`¡Buenas noticias! El aforo promedio ha subido <strong>${Math.round(tendencia)}%</strong> en las últimas semanas (${aforoSemanas[0]}% → ${aforoSemanas[aforoSemanas.length-1]}%).`,
        accion:'El programa va en buena dirección.',
        drillDown: `navegarA('dashboard')`
      });
    }
  }
  ...
  instructores.forEach(inst => {
    const impTotal = registros.filter(r=>r.inst_id===inst.id&&(r.estado==='ok'||r.estado==='sub')&&(r.fecha||'')>=hoy30str).length;
    const sups = registros.filter(r=>r.inst_id===inst.id&&r.estado==='sub'&&(r.fecha||'')>=hoy30str).length;
    if(impTotal>=4 && sups/impTotal >= 0.5){
      alertas.push({
        tipo:'gold',
        icon:'<svg class="ico" viewBox="0 0 20 20"><path d="M4 10a6 6 0 0 1 6-6 6 6 0 0 1 5.2 3" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round"/><path d="M16 10a6 6 0 0 1-6 6 6 6 0 0 1-5.2-3" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round"/><polyline points="14.5,7 15.5,3.8 18.5,5" stroke="currentColor" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/><polyline points="5.5,13 4.5,16.2 1.5,15" stroke="currentColor" stroke-width="1.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        texto:`<strong>${inst.nombre}</strong> tiene suplentes en el <strong>${Math.round(sups/impTotal*100)}%</strong> de sus clases este mes (${sups} de ${impTotal}).`,
        accion:'Considera revisar disponibilidad o reasignación de clases.',
        drillDown: `_abrirDetalleInstructor(${inst.id})`
      });
    }
  });
  ...
  const clasesConPocosDatos = Object.values(gruposClase).filter(g=>{
    const recientes = g.recs.filter(r=>(r.fecha||'')>=hoy30str);
    return recientes.length<3 && (instructores.find(i=>i.id===g.inst_id)?.horario||[]).length>0;
  });
  if(clasesConPocosDatos.length>2){
    alertas.push({
      tipo:'blue',
      icon:'<svg class="ico" viewBox="0 0 20 20"><rect x="3" y="12" width="3" height="5" rx="1" stroke="currentColor" stroke-width="1.4" fill="none"/><rect x="8.5" y="8" width="3" height="9" rx="1" stroke="currentColor" stroke-width="1.4" fill="none"/><rect x="14" y="4" width="3" height="13" rx="1" stroke="currentColor" stroke-width="1.4" fill="none"/></svg>',
      texto:`<strong>${clasesConPocosDatos.length} combinaciones</strong> de clase/instructor tienen menos de 3 registros en el último mes.`,
      accion:'Más datos permiten mejores diagnósticos. Registra clases regularmente.',
      drillDown: `navegarA('heatmap')`
    });
  }
...
// ── Helper: abrir detalle de instructor en el mapa de calor ──
function _abrirDetalleInstructor(instId) {
  // Navegar a heatmap y preseleccionar el instructor
  navegarA('heatmap');
  setTimeout(() => {
    const sel = document.getElementById('hm-inst-fil');
    if(sel) {
      sel.value = String(instId);
  ...
  container.innerHTML =
    ...
    alertas.map(a=>`
      <div class="alert-card alrt-${a.tipo}"
           style="cursor:${a.drillDown?'pointer':'default'};transition:opacity .15s,transform .1s"
           ${a.drillDown?`onclick="${a.drillDown}" onmouseover="this.style.opacity='.82';this.style.transform='scale(1.005)'" onmouseout="this.style.opacity='1';this.style.transform='scale(1)'"`:''}>
        <div class="alert-icon">${a.icon}</div>
        <div class="alert-txt">
          <div>${a.texto}</div>
          <div style="color:var(--txt3);font-size:.68rem;margin-top:3px;display:flex;justify-content:space-between;align-items:center">
            <span>→ ${a.accion}</span>
            ${a.drillDown?`<span style="color:var(--neon);font-size:.65rem;font-weight:600">Ver detalle →</span>`:''}
          </div>
        </div>
      </div>`).join('');
}